...
class ModelDownloader:
    ...
    def verify_download(self, model_size: str) -> bool:
        ...
        vocab_json = os.path.join(self.output_dir, "vocabulary.json")
        vocab_txt = os.path.join(self.output_dir, "vocabulary.txt")
        
        if os.path.exists(vocab_json):
            print(f"✅ 找到文件: vocabulary.json")
        elif os.path.exists(vocab_txt):
            print(f"✅ 找到文件: vocabulary.txt")
            # 创建 vocabulary.json 的副本
            try:
                import shutil
                shutil.copy2(vocab_txt, vocab_json)
                print(f"✅ 已创建 vocabulary.json 从 vocabulary.txt")
            except Exception as e:
                print(f"⚠️  无法创建 vocabulary.json: {e}")
        else:
            print(f"❌ 缺少文件: vocabulary.json 或 vocabulary.txt")
            return False
    ...
    def _print_manual_download_instructions(self, repo_id: str) -> None:
        ...
        print("   - vocabulary.txt 或 vocabulary.json")
        print("4. 运行验证: python scripts/setup_env.py") 
